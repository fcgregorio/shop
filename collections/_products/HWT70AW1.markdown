---
layout: product
name: Haier HWT70AW1 7kg Top Load Washer
brand: Haier
sku: HWT60AW1
price: $569
category: Washing Machines
subcategory: Top Load Washing Machines
details:
  Description:
    Features: ~
    Wash Action: Pulsating
    Lid: Soft Close
    Memory Program: yes
    Delay Start: yes
    Lint Filter: yes
    Specifications: ~
    Colour: White
    Dry load capacity (KG): 7
    Energy Consumption (kW): 459
    Energy rating: 2.5
    Water Consumption (L): 86
    Water rating (WELS): "3.5 Star, 86Lt Used Per Wash – WELS Product Registration Number: C01343"
    Number of Program cycles: 8
    Spin speed (max, rpm): 650
    Pulsating wash action: Yes
    Memory Program: Yes
    Electronic control with LED/Time Remaining display: Yes
    Progress Indicator: Yes
    Child Lock: Yes
    Soft Close Lid: Yes
    One Piece Tempered Glass Lid: Yes
    Delay start: Yes
    Adjustable front feet: Yes
    Lint filter: Yes
  Additional Information:
    Weight: 44 kg
    Dimensions: 63.8 × 63.4 × 103.5 cm
    Brand: Haier
    Warranty: 2 Years
    Height: 940
    Width: 550
    Depth: 570
    Energy Rating: 2.5
    Water Rating: 3.5
    Product Type: Laundry
    Product Features: Top Load Washers
    Product Category: Washing Machines
images:
  - xHWT70AW1-main.jpg.pagespeed.ic.2Akv2OuuWz.jpg
---